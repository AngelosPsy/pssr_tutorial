...
Here, we provide a brief tutorial for the 
[pssr](https://github.com/AngelosPsy/pssr) R package.
...
tool for experimental psychopathologists and other researchers acoss Clinical 
psychology, it can also be used from any researcher running experimental 
studies. 

Here, we first provide 
...
The rationale behind the design of the package can be found in our  
...
You can also choose between installing [MikTex](https://miktex.org/download) or 
[tinytex](https://yihui.name/tinytex/) -- _tinytex_ is lighter and seems to be 
...
have any problems during installation, please open an issue 
[here](https://github.com/AngelosPsy/pssr_tutorial/issues). Instructions 
...
To reopen an existing project the user can just click on the 
...
to use so as to form your preregistration. The _pssr_ software supports 
...
We describe what type of information should be included in each 
box in our paper. 
An example of a pre-registration can be found 
[below](#preexample).
...
'Encrypt Project files'. Please check with the your local ethic committee
under which conditions you are allowed to share the data. 

```{r  out.width = "80%", echo=FALSE}
include_graphics("figures/ed.png") 
```

# Example of using the _pssr_ software {#hexample}
...
in the preregistration. Also, we deviate from the preregistartion material in
order to provide a more detailed example. In some cases, we note, using the
word _note_, some parts of the preregistrations that could have been better.
Our goal is to show the reader that preregistrations that seem to be complete,
miss some important information that could have made them more detailed. We
extend on this issue also in our main paper.
...
Here, we first need to fill in the name of our project (i.e., Modality-specific 
dual-task interference on aversive memories). Then, we should click on the 
...
stored will be printed on screen (e.g., C/Desktop/ProjectName). 
Please note that everytime you want to open 
the projet, you can just click on the "Find existting project", locate 
...
by _pssr_, a warning will be returned. This will mean that also some functions
of the _pssr_ will not work (e.g., the zip function).
...
The first template was written by us and it is the template we describe 
in our relevant [paper](https://osf.io/yvnfj/). 
...
Then, click on the _Create_ button. After doing that, a new tab will be 
...
_Note_ As also mentioned in the paper, better hypotheses would also specify
the size of the effect -- something that is not done here. Also, the main
hypotheses do not include the operationalization of some of the key variables
(e.g., vividness, emotionality). This could be problematic given that we
could have taken different measures of these variables, and used only the
ones that satisfied our hypotheses.
...
_Note_ No power analysis has been performed here. Also, no other reason is given
as to why we selected this number.
---
...
_Note_ This could have been a typical case of yes/no logic. Since we use
Bayesian analyses, it could have been better to also specificy the size of
effect that provide insufficient evidence for (dis-)confirming our hypotheses.
...
For the sake of the example, we will open the "exampleDataSet.csv" file. 
...
reason, we are afraid could trace some of our participants back based 
on their age -- a bit unrealistic argument in the present case. 
In order to fill in this column 
...
Although this is descibed as the sixth step, the "_Record changes_" tab 
should be visited often during the whole project and not just in the end. 
Specifically, it is best to 
...
feature of the software to work. 
In case of no commits, the tab will return an error.